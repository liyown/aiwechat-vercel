...
func (chat *QwenChat) chat(userId string, message string) (res string) {
	...
	if chat.maxTokens > 0 {
		qwenReq.MaxTokens = chat.maxTokens // 参数名称参考：https://help.aliyun.com/zh/dashscope/developer-reference/api-details
	}
	qwenReq.TopP = 0.8              // 通义千问要求top_p ∈ (0,1)
	qwenReq.RepetitionPenalty = 1.1 // 用于控制模型生成时的重复度，需要大于0。提高repetition_penalty时可以降低模型生成的重复度。1.0表示不做惩罚。默认为1.1。
	qwenReq.Temperature = 0.85      // 取值范围：[0, 2)，系统默认值0.85。不建议取值为0，无意义。
